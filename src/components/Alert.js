import { 
  AlertDialog, 
  AlertDialogBody, 
  AlertDialogContent, 
  AlertDialogHeader, 
  AlertDialogOverlay, 
} from "@chakra-ui/react"; 
import { useAlertContext } from "../context/alertContext"; 
import { useRef } from "react"; 
import { useTranslation } from 'react-i18next'; // Importando o hook de tradução

function Alert() { 
  const { isOpen, type, message, onClose } = useAlertContext(); 
  const { t } = useTranslation(); // Usando o hook de tradução
  const cancelRef = useRef(); 
  const isSuccess = type === "success";

  // Usando o tipo de mensagem correta
  const messageContent = isSuccess 
    ? t('alert.successMessage', { name: 'Igor' }) // Mensagem de sucesso com nome
    : t('alert.errorHeader'); // Mensagem de erro sem interpolação, pois a interpolação é só para sucesso

  return ( 
    <AlertDialog 
      isOpen={isOpen} 
      leastDestructiveRef={cancelRef} 
      onClose={onClose} 
    > 
      <AlertDialogOverlay> 
        <AlertDialogContent py={4} backgroundColor={isSuccess ? '#81C784' : '#FF8A65'}> 
          <AlertDialogHeader fontSize="lg" fontWeight="bold"> 
            {isSuccess ? t('alert.successHeader') : t('alert.errorHeader')} 
          </AlertDialogHeader> 
          <AlertDialogBody>{messageContent}</AlertDialogBody> 
        </AlertDialogContent> 
      </AlertDialogOverlay> 
    </AlertDialog> 
  ); 
}

export default Alert;